from __future__ import division, absolute_import, print_function
import pytest


def test_resp_1():
    import qcdb
    import resp
    import numpy as np

    # Set basis set
    qcdb.set_options({'basis': 'GBASIS=N31 NGAUSS=6 NDFUNC=1'})

    # Initialize molecule
    mol = qcdb.Molecule.from_string(""" C   1.45051389  -0.06628932   0.00000000
     H   1.75521613  -0.62865986  -0.87500146
     H   1.75521613  -0.62865986   0.87500146
     H   1.92173244   0.90485897   0.00000000
     C  -0.04233122   0.09849378   0.00000000
     O  -0.67064817  -1.07620915   0.00000000
     H  -1.60837259  -0.91016601   0.00000000
     O  -0.62675864   1.13160510   0.00000000""")
    mol.update_geometry()

    # Set options
    options = {'RESP_N_VDW_LAYERS'       : 4,
               'RESP_VDW_SCALE_FACTOR'   : 1.4,
               'RESP_VDW_INCREMENT'      : 0.2,
               'RESP_VDW_POINT_DENSITY'  : 1.0,
               'RESP_RESP_A'             : 0.0005,
               'RESP_RESP_B'             : 0.1,
               'RESP_QM_PACKAGE'         : 'GAMESS'
               }

    qcdb.set_options(options)
    # Call for first stage fit
    charges1 = resp.resp('SCFTYP=RHF', [mol])
    print('Electrostatic Potential Charges')
    print(charges1[0][0])
    print('Restrained Electrostatic Potential Charges')
    print(charges1[0][1])
    # Reference charges are generated by the R.E.D.-III.5 tools
    # with GAMESS as the quantum chemistry package
    reference_charges1 = np.array([-0.294974,  0.107114,  0.107114,  0.084795,
                                    0.803999, -0.661279,  0.453270, -0.600039])
    print('Reference RESP Charges')
    print(reference_charges1)
    print('Difference')
    print(charges1[0][1]-reference_charges1)

    assert np.allclose(charges1[0][1], reference_charges1, atol=5e-4)

    # Change the value of the RESP parameter A
    options['RESP_RESP_A'] = 0.001

    # Add constraint for atoms fixed in second stage fit
    constraint_charge = []
    for i in range(4, 8):
        constraint_charge.append([charges1[0][1][i], [i+1]])
    options['RESP_CONSTRAINT_CHARGE'] = [constraint_charge]
    options['RESP_CONSTRAINT_GROUP'] = [[[2, 3, 4]]]
    options['RESP_GRID'] = ['1_%s_grid.dat' %mol.name()]
    options['RESP_ESP'] = ['1_%s_grid_esp.dat' %mol.name()]
    qcdb.set_options(options)
    mol.set_name('stage2')

    # Call for second stage fit
    charges2 = resp.resp('SCFTYP=RHF', mol)

    # Get RESP charges
    print("\nStage Two:\n")
    print('RESP Charges')
    print(charges2[0][1])
    reference_charges2 = np.array([-0.290893,  0.098314,  0.098314,  0.098314,
                                   0.803999, -0.661279,  0.453270, -0.600039])
    print('Reference RESP Charges')
    print(reference_charges2)
    print('Difference')
    print(charges2[0][1]-reference_charges2)

    assert np.allclose(charges2[0][1], reference_charges2, atol=5e-4)
    qcdb.driver.pe.clean_nu_options()


def test_resp_2():
    import qcdb
    import resp
    import numpy as np

    # Set basis set
    qcdb.set_options({'basis': '6-31g*'})

    # Initialize molecule
    mol = qcdb.Molecule.from_string(""" C   1.45051389  -0.06628932   0.00000000
     H   1.75521613  -0.62865986  -0.87500146
     H   1.75521613  -0.62865986   0.87500146
     H   1.92173244   0.90485897   0.00000000
     C  -0.04233122   0.09849378   0.00000000
     O  -0.67064817  -1.07620915   0.00000000
     H  -1.60837259  -0.91016601   0.00000000
     O  -0.62675864   1.13160510   0.00000000""")
    mol.update_geometry()

    # Set options
    options = {'RESP_N_VDW_LAYERS'       : 4,
               'RESP_VDW_SCALE_FACTOR'   : 1.4,
               'RESP_VDW_INCREMENT'      : 0.2,
               'RESP_VDW_POINT_DENSITY'  : 1.0,
               'RESP_RESP_A'             : 0.0005,
               'RESP_RESP_B'             : 0.1,
               'RESP_QM_PACKAGE'         : 'Q-CHEM'
               }

    qcdb.set_options(options)
    # Call for first stage fit
    charges1 = resp.resp('HF', mol)
    print('Electrostatic Potential Charges')
    print(charges1[0][0])
    print('Restrained Electrostatic Potential Charges')
    print(charges1[0][1])
    # Reference charges are generated by the R.E.D.-III.5 tools
    # with GAMESS as the quantum chemistry package
    reference_charges1 = np.array([-0.294974,  0.107114,  0.107114,  0.084795,
                                    0.803999, -0.661279,  0.453270, -0.600039])
    print('Reference RESP Charges')
    print(reference_charges1)
    print('Difference')
    print(charges1[0][1]-reference_charges1)

    assert np.allclose(charges1[0][1], reference_charges1, atol=5e-4)

    # Change the value of the RESP parameter A
    options['RESP_RESP_A'] = 0.001

    # Add constraint for atoms fixed in second stage fit
    constraint_charge = []
    for i in range(4, 8):
        constraint_charge.append([charges1[0][1][i], [i+1]])
    options['RESP_CONSTRAINT_CHARGE'] = [constraint_charge]
    options['RESP_CONSTRAINT_GROUP'] = [[[2, 3, 4]]]
    options['RESP_GRID'] = ['1_%s_grid.dat' %mol.name()]
    options['RESP_ESP'] = ['1_%s_grid_esp.dat' %mol.name()]
    qcdb.set_options(options)
    mol.set_name('stage2')

    # Call for second stage fit
    charges2 = resp.resp('HF', [mol])

    # Get RESP charges
    print("\nStage Two:\n")
    print('RESP Charges')
    print(charges2[0][1])
    reference_charges2 = np.array([-0.290893,  0.098314,  0.098314,  0.098314,
                                   0.803999, -0.661279,  0.453270, -0.600039])
    print('Reference RESP Charges')
    print(reference_charges2)
    print('Difference')
    print(charges2[0][1]-reference_charges2)

    assert np.allclose(charges2[0][1], reference_charges2, atol=5e-4)
    qcdb.driver.pe.clean_nu_options()


def test_resp_3():
    import qcdb
    import resp
    import numpy as np

    # Set basis set
    qcdb.set_options({'basis': '6-31g*'})

    # Initialize two different conformations of ethanol
    geometry = """C    0.00000000  0.00000000  0.00000000
    C    1.48805540 -0.00728176  0.39653260
    O    2.04971655  1.37648153  0.25604810
    H    3.06429978  1.37151670  0.52641124
    H    1.58679428 -0.33618761  1.43102358
    H    2.03441010 -0.68906454 -0.25521028
    H   -0.40814044 -1.00553466  0.10208540
    H   -0.54635470  0.68178278  0.65174288
    H   -0.09873888  0.32890585 -1.03449097
    """
    mol1 = qcdb.Molecule.from_string(geometry)
    mol1.set_name('conformer1')
    mol1.update_geometry()

    geometry = """C    0.00000000  0.00000000  0.00000000
    C    1.48013500 -0.00724300  0.39442200
    O    2.00696300  1.29224100  0.26232800
    H    2.91547900  1.25572900  0.50972300
    H    1.61500700 -0.32678000  1.45587700
    H    2.07197500 -0.68695100 -0.26493400
    H   -0.32500012  1.02293415 -0.30034094
    H   -0.18892141 -0.68463906 -0.85893815
    H   -0.64257065 -0.32709111  0.84987482
    """
    mol2 = qcdb.Molecule.from_string(geometry)
    mol2.set_name('conformer2')
    mol2.update_geometry()

    molecules = [mol1, mol2]

    # Specify intermolecular constraints
    intermolecular_constraint = {'EQUAL': [[[1, range(1, 10)], [2, range(1, 10)]]]}

    # Specify options
    options = {'RESP_N_VDW_LAYERS'       : 4,
               'RESP_VDW_SCALE_FACTOR'   : 1.4,
               'RESP_VDW_INCREMENT'      : 0.2,
               'RESP_VDW_POINT_DENSITY'  : 1.0,
               'RESP_RESP_A'             : 0.0005,
               'RESP_RESP_B'             : 0.1,
               'RESP_RESTRAINT'          : True,
               'RESP_IHFREE'             : False,
               'RESP_WEIGHT'             : 1.,
               'RESP_QM_PACKAGE'         : 'PSI4'
               }
    qcdb.set_options(options)

    # Call for first stage fit
    charges1 = resp.resp('scf', molecules, intermolecular_constraint)

    print("Restrained Electrostatic Potential Charges")
    print(charges1[0][1])
    # Reference Charges are generates with the resp module of Ambertools
    # Grid and ESP values are from this code with Psi4
    reference_charges1 = np.array([-0.149134, 0.274292, -0.630868,  0.377965, -0.011016,
                                   -0.009444,  0.058576,  0.044797,  0.044831])
    print("Reference RESP Charges")
    print(reference_charges1)
    print("Difference")
    print(charges1[0][1]-reference_charges1)

    assert np.allclose(charges1[0][1], reference_charges1, atol=1e-5)

    # Add constraint for atoms fixed in second stage fit
    stage2 = resp.stage2_helper()
    options['RESP_RESP_A'] = 0.001
    options['RESP_GRID'], options['RESP_ESP'] = [[]]*2, [[]]*2
    options['RESP_CONSTRAINT_CHARGE'], options['RESP_CONSTRAINT_GROUP'] = [[]]*2, [[]]*2
    for imol in range(len(molecules)):
        stage2.set_stage2_constraint(molecules[imol], charges1[imol][1], cutoff=1.2)
        options['RESP_CONSTRAINT_CHARGE'][imol] = stage2.constraint_charge
        options['RESP_CONSTRAINT_GROUP'][imol] = stage2.constraint_group
        options['RESP_GRID'][imol] = '%i_%s_grid.dat' %(imol+1, molecules[imol].name())
        options['RESP_ESP'][imol] = '%i_%s_grid_esp.dat' %(imol+1, molecules[imol].name())
        molecules[imol].set_name('conformer' + str(imol+1) + '_stage2')
    qcdb.set_options(options)

    # Add intermolecular constraints
    stage2.stage2_intermolecular_constraint(molecules, cutoff=1.2)

    # Call for second stage fit
    print(molecules)
    print(options)
    print(stage2.intermolecular_constraint)
    charges2 = resp.resp('scf', molecules, stage2.intermolecular_constraint)
    print("\nStage Two\n")
    print("RESP Charges")
    print(charges2[0][1])
    reference_charges2 = np.array([-0.079853, 0.253918, -0.630868, 0.377965, -0.007711,
                                   -0.007711, 0.031420,  0.031420, 0.031420])
    print("Reference RESP Charges")
    print(reference_charges2)
    print("Difference")
    print(charges2[0][1]-reference_charges2)

    assert np.allclose(charges2[0][1], reference_charges2, atol=1e-5)
    qcdb.driver.pe.clean_nu_options()
